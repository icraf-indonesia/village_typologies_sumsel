...
# Subset village_metadata
village_metadata_banyuasin <- village_metadata %>%
...
cluster_lookup_<- tibble(village_metadata_banyuasin
) %>% 
  left_join(lookup_cluster, by="cluster") %>% 
  rename(Typology = name)

tp_map <- create_leaflet_map(clusters_kec = "output/cluster_map_Banyuasin.shp", cluster_lookup = cluster_lookup_, col_names = "k_8") %>% bslib::card(full_screen = TRUE)
