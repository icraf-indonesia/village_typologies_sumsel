...
# Save the workbook
saveWorkbook(wb, file = "output/summary_table_sumsel_colored.xlsx", overwrite = TRUE)
...
k_5 <- st_read("output/cluster_map_SumSel.shp") %>%
  pull(k_5) %>%
  tibble(cluster = .)
...
# Save output
write_xlsx(summary_df_long, "output/summary_table_sumsel_k5.xlsx")
...
# Save the workbook
saveWorkbook(wb, file = "output/summary_table_sumsel_colored_k5.xlsx", overwrite = TRUE)
